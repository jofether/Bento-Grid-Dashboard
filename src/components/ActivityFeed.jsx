...
export function ActivityFeed({ darkMode }) {
  const [activities, setActivities] = useState([
    ...
    { id: 3, user: 'Emma Davis', action: 'merged PR', time: '1h ago', icon: '✅' },
    { id: 4, user: 'Jake Wilson', action: 'reviewed code', time: '2h ago', icon: '👁️' },
  ]);
    ...
    <div className={`${darkMode ? 'bg-neutral-800/50' : 'bg-white'} backdrop-blur-xl rounded-3xl p-6 border ${darkMode ? 'border-neutral-700/50' : 'border-gray-200/50'}`}>
      ...
      <div className="space-y-3">
        {activities.map((activity) => (
          <div key={activity.id} className="flex items-center gap-3 pb-3 border-b border-neutral-700/30 last:border-0">
